...
package com.siloon.containers.table.grid
{

import com.siloon.containers.table.Cell;
import com.siloon.containers.table.constant.CellConst;
import com.siloon.containers.table.grid.gridClasses.GridColumnInfo;
import com.siloon.containers.table.grid.gridClasses.GridRowInfo;
...
public class Grid extends Box
{
     ...
     */
    table_internal var columnWidths:Array /* of GridColumnInfo */;

    /**
     *  @private
     *  Minimum, maximum, and preferred height of each row.
     */
    table_internal var rowHeights:Array /* of GridRowInfo */;
    ...
    override protected function measure():void
    {
        ...
        for (i = 0; i < numRows; i++)
        {
            ...
            gridRow.gridRowIndex = gridRowIndex;
            minRowSpan = int.MAX_VALUE;
            ...
            {           
                ...
                if (gridRowIndex > 0)
                {
                    var occupied:int = columnOccupiedUntilRow[gridColIndex];
                    while (!isNaN(occupied) && occupied >= gridRowIndex)
                    {
                        gridColIndex++;
                        occupied = columnOccupiedUntilRow[gridColIndex];
                    }
                } 
                ...
                gridItem.gridColIndex = gridColIndex;

                // If this cell extends to rows beyond this one, remember
                // which columns are occupied.
                if (gridItem.gridRowSpan > 1)
                {
                    var lastRowOccupied:int = gridRowIndex + gridItem.gridRowSpan - 1;
                    for (var k:int = 0; k < gridItem.gridColSpan; k++)
                    {
                        columnOccupiedUntilRow[gridColIndex + k] = lastRowOccupied;
                    }
                } 

                gridColIndex += gridItem.gridColSpan;
                
                minRowSpan = Math.min(minRowSpan,gridItem.gridRowSpan);
            }

            if (gridColIndex > numGridColumns)
                numGridColumns = gridColIndex;
             
            gridRowIndex = gridRowIndex + minRowSpan;
                
            if(gridRowIndex > numGridRows)
            {
            	numGridRows = gridRowIndex;
            }
        }
        ...
        {
            gridRow = rowList[i];
            gridRow.gridColumnWidths = columnWidths;
            gridRow.gridRowHeights = rowHeights;
            
            for (j = 0; j < gridRow.numGridItems; j++)
            {
            	gridItem = GridItem(gridRow.getChildAt(j));
                var rowSpan:int = gridItem.gridRowSpan;
                var colSpan:int = gridItem.gridColSpan;
                
                distributeItemHeight(gridItem, i, verticalGap, rowHeights);
                distributeItemWidth(gridItem, gridItem.gridColIndex,
        ...
        if (numGridColumns > 1)
            padding += getStyle("verticalGap") * (numGridColumns - 1);
    ...
    {       
        ...
        // Set the row heights, which were calculated by the VBox layout
        // algorithm, into the rowHeights array.      
        for ( i = 0; i < rowHeights.length ; i++)
        {
        	rowHeights[i].y = i * CellConst.BASIC_HEIGHT;
            rowHeights[i].height = CellConst.BASIC_HEIGHT;
        }
                                          ...
                                          rowHeights:Array):void
    {
        ...
        var rowSpan:int = item.gridRowSpan;
                                         ...
                                         horizontalGap:Number, columnWidths:Array):void
    {
        ...
        var colSpan:int = item.gridColSpan;
