...
public class GridRow extends HBox
{
     ...
     */
    table_internal var gridColumnWidths:Array /* of GridColumnInfo */;

    /**
     *  @private
     *  Height of rows.
     */
    table_internal var gridRowHeights:Array /* of GridRowInfo */;

    /**
     *  @private
     *  Index of this row.
     */
    table_internal var gridRowIndex:int = 0;
    ...
    {
        var numChildren:Number = gridColumnWidths.length;
        ...
        {
            var child:GridColumnInfo = gridColumnWidths[i];
            minWidth += child.min;
        ...
        var rowInfo:GridRowInfo = gridRowHeights[gridRowIndex];
    ...
    {       
        ...
        if (parent.getChildIndex(this) == 0 ||
            isNaN(gridColumnWidths[0].x) ||
            gridColumnWidths.minWidth != minWidth ||
            gridColumnWidths.maxWidth != maxWidth ||
            gridColumnWidths.preferredWidth != getExplicitOrMeasuredWidth() ||
            gridColumnWidths.percentWidth != percentWidth ||
            gridColumnWidths.width != unscaledWidth ||
            gridColumnWidths.paddingLeft != getStyle("paddingLeft") ||
            gridColumnWidths.paddingRight != getStyle("paddingRight") ||
            gridColumnWidths.horizontalAlign != getStyle("horizontalAlign") ||
            gridColumnWidths.borderStyle != getStyle("borderStyle"))
        {
        ...
        for (var i:int = 0; i < n; i++)
        {
            ...
            var colIndex:int = child.gridColIndex;
            var x:Number = gridColumnWidths[colIndex].x;
            var y:Number = vm.top;
            var percentHeight:Number = child.percentHeight;

            var finalColIndex:int =
                Math.min(colIndex + child.gridColSpan, gridColumnWidths.length);
            var finalCol:GridColumnInfo = gridColumnWidths[finalColIndex - 1];
            var w:Number = finalCol.x + finalCol.width - x;

            var finalRowIndex:int =
                Math.min(gridRowIndex + child.gridRowSpan, gridRowHeights.length);
            var finalRow:GridRowInfo = gridRowHeights[finalRowIndex - 1];
            var h:Number = finalRow.y + finalRow.height -
                           gridRowHeights[gridRowIndex].y - vm.top - vm.bottom;
    ...
    private function calculateColumnWidths():void
    {
        ...
        var numChildren:int = gridColumnWidths.length;
